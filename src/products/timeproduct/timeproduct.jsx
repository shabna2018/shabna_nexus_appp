...
const data = Array.from({ length: 12 }, () => ({
  ...
  faceValue: "$90.00",
  sellingPrice: "$100.00",
timeBalance: 15, // or 5, 0 depending on row
    ...
    <div className="time-product-table-wrapper">
      ...
      <div className="time-product-section-header">
        <div className="time-product-section-title-container">
          ...
          <img src={timeIcon} alt="Card Icon" className="time-product-section-icon" /> 
          ...
          <div className="time-product-checkbox-container">
            ...
            />
            <label htmlFor="activeOnly" className="time-product-checkbox-label22">
      ...
      <div className="time-product-header-actions">
        ...
        <div className="time-product-search-container2">
          <img src={searchIcon} alt="Search" className="time-product-search-input2-icon" />
          <input
            type="text"
            placeholder="Search"
            value={productFilter}
        ...
        <div className="time-product-filter-container">
          ...
          >
            <img src={filterIcon} alt="Filter" className="time-product-filter-icon" />
          </button>
          {showFilterDropdown && (
        ...
        <button className="time-product-btn-search">
          <img src={glass2Icon} alt="Search" className="time-product-search-btn-icon" />
          Search
        </button>
        ...
        <div className="time-product-action-buttons">
           <button 
            ...
            onClick={() => navigate("/time-productform")} // ✅ navigate on click
          >
            Add
          </button>

          <button className="time-product-btn-refresh">Refresh</button>
        </div>
      </div>
          ...
          <tbody>
            {data.map((row, index) => (
              <tr key={index}>
                ...
                <td>{row.timeBalance}</td>
                <td>{row.cardQuantity}</td>
                <td>
                  {row.accessProfile === "attraction: content" ? (
                    <div className="time-product-text-box">{row.accessProfile}</div>
                  ) : (
